...
public class PayController {
    ...
    @LogOperation("支付回调通知")
    @PostMapping("/notify/order/{paramCode}")
    @AnonAccess
    public String parseOrderNotifyResult(@ApiParam(name = "paramCode", value = "配置参数", required = true) @PathVariable String paramCode,
                                         @RequestBody String xmlData) {
        /*try {
            WxPayOrderNotifyResult notifyResult = getWxService(paramCode).parseOrderNotifyResult(xmlData);
            if ("SUCCESS".equalsIgnoreCase(notifyResult.getResultCode()) && "SUCCESS".equalsIgnoreCase(notifyResult.getResultCode())) {
                orderService.payNotify(ShopConst.PayTypeEnum.WECHAT.value(),
                        notifyResult.getOutTradeNo(),
                        notifyResult.getTransactionId(),
                        new BigDecimal(notifyResult.getTotalFee()).divide(new BigDecimal(100), RoundingMode.HALF_UP),
                        DateUtils.parse(notifyResult.getTimeEnd(), "yyyyMMddHHmmss"));
            }
     ...
     *
     * 注意以下两个参数只能并且必须传一个
     * transactionId 微信订单号,支付回调中返回
     * outTradeNo    商户系统内部的订单号
     */
    @ApiOperation(value = "查询订单")
    @PostMapping("/queryOrder/{paramCode}")
    @RequiresPermissions("shop:order:info")
    public Result<?> queryOrder(@ApiParam(name = "paramCode", value = "配置参数", required = true) @PathVariable String paramCode,
                                @RequestBody WxPayOrderQueryRequest wxPayOrderQueryRequest) {
        try {
            WxPayOrderQueryResult result = getWxService(paramCode).queryOrder(wxPayOrderQueryRequest);
    ...
    @PostMapping("/refund/{paramCode}")
    @RequiresPermissions("shop:order:update")
    public Result<?> refund(@ApiParam(name = "paramCode", value = "配置参数", required = true) @PathVariable String paramCode,
                            @RequestBody WxPayRefundRequest request) {
        try {
            WxPayRefundResult result = getWxService(paramCode).refund(request);
            return new Result<>().success(result);
        } catch (WxPayException e) {
            e.printStackTrace();
            throw new OnexException(ErrorCode.WX_API_ERROR, e);
    ...
    public <T> T createOrder(@RequestBody WxPayUnifiedOrderRequest request, @RequestParam(defaultValue = "WX-") String paramCode) throws WxPayException {
        return getWxService(paramCode).createOrder(request);
    }
     ...
     * @return 微信Service
     */
    private WxPayService getWxService(String paramCode) {
        // 从参数表获取参数配置
        WxPayProp wxProp = paramService.getContentObject(paramCode, WxPayProp.class, null);
